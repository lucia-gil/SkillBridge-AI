...
public interface AsignacionRepository extends JpaRepository<Asignacion, Long> {
    ...
    // Todas las asignaciones activas con perfil+usuario ya cargados, para
    // construir el mapa de carga semanal (resource-manager/inicio.html) sin
    // golpear la BD una vez por colaborador.
    @Query("select a from Asignacion a join fetch a.perfil p join fetch p.usuario u where a.estado = 'activa'")
    List<Asignacion> listarActivasConPerfil();
}
